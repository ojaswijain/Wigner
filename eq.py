...
def belltest(lmax,nrLoops,start,end):
    ...
    ##################################################################################
### conditions lmax is defined in belltest(lmax, nrlooos, star, end)
            ...
            for l2 in range(0, lmax+1):
                for l3 in range(0, lmax+1):
                    if((l1<=l2<=l3) and (abs(l1-l2)<=l3<=abs(l1+l2)) and ((l1+l2+l3)==l0)):
                        ...
                        # anuncio("output value of Gaunt:")
                        # anuncio(float(sum))
                        # anuncio("Calculated with values -> i="+str(i)+" l1="+str(l1)+" l2="+str(l2)+" l3="+str(l3))
                        
                        B_ell.append(sum)
                        
                        # anuncio("State of the matrix B_ell")
                        # anuncio(np.array(B_ell))
                        
                        # np.savetxt('B_ell_Cube_21_L0_Smooth_channel_'+str(i)+'.txt', B_ell, delimiter=',')
    #np.savetxt('B_ellRecalculado'+str(i)+".txt", B_ell, delimiter=',')
                        # np.savetxt('B_ell_ell_Cube_21_L0_Smooth_channel_'+str(i)+'.txt', np.array([B_ell,l1_ell,l2_ell, l3_ell]).T, delimiter=',')
    ...
    # print("Saving Files")
    #np.savetxt('B_ell_Reconstructed_Teste1_equisize_Redshift_'+str(i)+'.txt', B_ell, delimiter=',')
    #np.savetxt('B_ellRecalculado'+str(i)+".txt", B_ell, delimiter=',')
    #np.savetxt('B_ell_ell_Reconstructed_Teste1_equisize_Redshift_'+str(i)+'.txt', np.array([B_ell,l1_ell,l2_ell, l3_ell]).T, delimiter=',')
