...
import time
import pipeline
from plotter import plotter, plot_time, heatmap
import pandas as pd
import utils
...
def wigner_sol(wigner3j):
    ...
    val1 = pipeline.give_val_ana(wig1[0], wig1[1])
    val3 = pipeline.give_val_ana(wig3[0], wig3[1])

    val = -1*(val1*coeff[0]+val3*coeff[2])/coeff[1]

    pipeline.store_val_ana(np.array(wigner3j[0]), np.array(wigner3j[1]), val)

	#Comment out everything except line 210 to measure true speed of algorithm
    print_lll(wigner3j)
    w = float(wigner_3j(*wigner3j[0],*wigner3j[1]))
    e = float((w-val)/w)
    ...
    x.append(np.abs(wigner3j[1]).min())
...
if __name__ == "__main__":
    ...
    for m in range(1,51,1):
        ...
        for i in range(-1,-m,-1):
            j=-i-m
            wigner0 = np.array([[120,130,140],[i,j,m]])
            wigner_sol(wigner0)
            wigner0 = np.array([[120,130,140],[-i,-j,-m]])
            wigner_sol(wigner0)
        for i in range(-1,-m,-1):
            j=-i-m
            wigner0 = np.array([[120,130,140],[m,i,j]])
            wigner_sol(wigner0)
            wigner0 = np.array([[120,130,140],[-m,-i,-j]])
            wigner_sol(wigner0)
        for i in range(-1,-m,-1):
            j=-i-m
            wigner0 = np.array([[120,130,140],[i,m,j]])
            wigner_sol(wigner0)
            wigner0 = np.array([[120,130,140],[-i,-m,-j]])
            wigner_sol(wigner0)
        
        wigner0=np.array([[120,130,140],[0,-m,m]])
        wigner_sol(wigner0)
        wigner0=np.array([[120,130,140],[0,m,-m]])
        wigner_sol(wigner0)
        wigner0=np.array([[120,130,140],[m,0,-m]])
        wigner_sol(wigner0)
        wigner0=np.array([[120,130,140],[-m,0,m]])
        wigner_sol(wigner0)
        wigner0=np.array([[120,130,140],[m,-m,0]])
        wigner_sol(wigner0)
        wigner0=np.array([[120,130,140],[-m,m,0]])
        wigner_sol(wigner0)
        end = time.time()
    ...
    plot_time(v,t,"time_simple","Blue")