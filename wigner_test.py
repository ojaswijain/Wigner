	...
	for i, wigs in enumerate(iteration_3js):
		...
		for j, wig in enumerate(wigs):
			...
			if wig in known_3js[0:-1]:
				any_known = True
				wig_value = wigner_3j(*wig[0],*wig[1]) # TO-OJASWI: this line
...
def find_wigner(wigner0, print_recursion=True, 
	...
	if print_results:
		...
		for i, wigner in enumerate(Wigners):
			print_lll(wigner)
			w = float(wigner_3j(*wigner[0],*wigner[1])) # TO-OJASWI: this line
...
if __name__=="__main__":
	...
	known_3js = [[list(wigner0[0]),[-2,0,2]],
				 [list(wigner0[0]),[-2,2,0]],
				 [list(wigner0[0]),[0,-2,2]],
				 [list(wigner0[0]),[0,0,0]],
				 [list(wigner0[0]),[0,2,-2]],
				 [list(wigner0[0]),[2,-2,0]],
				 [list(wigner0[0]),[2,0,-2]],
				 wigner0.tolist()]
